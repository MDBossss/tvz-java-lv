...
;            }
    ...
    public static Map<Long,Student> getStudentwithBirthday() throws BazaPodatakaException{
        Map<Long,Student> studentiMap = new HashMap<>();
        try(Connection connection = connectToDatabase()){
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM STUDENT WHERE EXTRACT(MONTH FROM DATUM_RODJENJA) = ? AND EXTRACT(DAY FROM DATUM_RODJENJA) = ?;");
            preparedStatement.setString(1, String.valueOf(LocalDateTime.now().getMonthValue()));
            preparedStatement.setString(2,String.valueOf(LocalDateTime.now().getDayOfMonth()));
