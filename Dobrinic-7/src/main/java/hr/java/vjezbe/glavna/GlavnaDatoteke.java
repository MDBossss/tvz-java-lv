...
public class GlavnaDatoteke {
    ...
    public static void main(String[] args){
        Map<Long, Profesor> profesori = getProfesori(PROFESORI_FILE);
        Map<Long, Student> studenti = getStudenti(STUDENTI_FILE);
        Map<Long, Predmet> predmeti = getPredmeti(PREDMETI_FILE,profesori,studenti);
        Map<Long, Ispit> ispiti = getIspiti(ISPITI_FILE,studenti,predmeti);
        Map<Long, ObrazovnaUstanova> obrazovneUstanove = getObrazovneUstanove(OBRAZOVNE_USTANOVE_FILE,profesori,predmeti,ispiti);
        ...
        for (var obrazovnaUstanova : obrazovneUstanove.values()) {
            ...
            var izvrsni = obrazovnaUstanova.getOdlicneIspite();
    ...
    static Map<Long,ObrazovnaUstanova> getObrazovneUstanove(String filename,Map<Long,Profesor> sviProfesori,Map<Long,Predmet> sviPredmeti,Map<Long,Ispit> sviIspiti){
...
// specific utility
    ...
    static Map<Profesor, List<Predmet>> dobijNositelje(Collection<Predmet> predmeti) {
        ...
        for (var predmet : predmeti) {
            Profesor nositelj = (Profesor) predmet.getNositelj();
    ...
    static Map<Student, BigDecimal> dobijKonacneOcjeneStudenata(List<Ispit> ispiti) {
        ...
        return ispitiStudenata.entrySet().stream().map(entry -> {
            ...
            for (var ispit : entry.getValue()) {
                sum = sum.add(new BigDecimal(ispit.getOcjena().vrijednost));
                count++;
            }
